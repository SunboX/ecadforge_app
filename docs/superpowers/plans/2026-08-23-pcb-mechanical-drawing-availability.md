...
**Goal:** Show the PCB mechanical-drawings checkbox only when the active PCB contains renderable technical drawing content.

**Architecture:** Add a content-aware drawing-layer resolver to `PcbLayerVisibilityModel` and reuse it in both checkbox rendering and default hidden-state initialization. Match generic primitive layer identifiers against normalized drawing-layer aliases without file-specific logic.
...
- Empty or malformed primitive collections count as no drawing content.
- Keep all source and test files below 1000 lines and add JSDoc for every function.
- Release as ECAD Forge 1.13.30 together with the already prepared toolkit integration in the shared worktree.

---

### Task 1: Content-aware drawing layer resolution
...
- Produces: `resolvePopulatedMechanicalDrawingLayerKeys(documentModel): string[]`

- [ ] **Step 1: Write the failing model tests**

Add one PCB whose declared mechanical and notes layers contain tracks and text,
and one PCB with the same declared layers but empty primitive collections.
Assert the content-aware resolver returns both populated keys for the first
board and an empty list for the second.

- [ ] **Step 2: Verify the tests fail for missing content awareness**

Run: `node --test tests/core/pcb-layer-visibility-model.test.mjs`

Expected: FAIL because `resolvePopulatedMechanicalDrawingLayerKeys` does not
exist.

- [ ] **Step 3: Implement the minimal generic resolver**
...
`polygons`, `texts`, `dimensions`, and `coordinates`; mark a drawing layer as
populated when a primitive layer reference matches one of its aliases.
...
- Consumes: `resolvePopulatedMechanicalDrawingLayerKeys(documentModel): string[]`
- Produces: checkbox markup only for populated drawing layers and matching default hidden state

- [ ] **Step 1: Write failing observable behavior tests**

Assert `PcbViewRenderer.render()` omits `data-pcb-mechanical-drawings` for a
board with empty declared mechanical layers. Extend controller coverage so an
empty drawing layer declaration does not create hidden-layer state.
...
Use `resolvePopulatedMechanicalDrawingLayerKeys()` in the checkbox renderer and
...
- Modify: `docs/release-notes-v1.13.30.md`
- Preserve: existing 1.13.30 package, lockfile, structured-data, and toolkit integration changes

**Interfaces:**

- Consumes: verified app source and tests
- Produces: ECAD Forge v1.13.30 GitHub release and successful production deployment
...
- [ ] **Step 2: Commit and publish the complete intended 1.13.30 app release**

Review `git diff`, commit intended app changes with an imperative prefixed
message, push `main`, and create GitHub release `v1.13.30` from the pushed SHA.
...
checkbox. Confirm the deployed footer reports version 1.13.30 and the console
