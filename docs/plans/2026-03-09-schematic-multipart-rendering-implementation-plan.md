...
Add a test that parses `tests/fixtures/altium/Skylace-Nova.SchDoc` and asserts the sheet resolves to `A3`, the active `WYRN2` parts expose `Wyrm Gate`, `Ember Well`, and `Skylace / Echo` text exactly once, and the visible `WYRN2` pin count stays in the expected narrowed range.
...
Add a renderer test for the same file that asserts the body-outline lines from record `6` appear in the SVG and that `Wyrm Gate`, `Ember Well`, and `Skylace / Echo` each render once.
