...
- Verify native parser output against real sample `.SchDoc` and `.PcbDoc` files
...
- `tests/core/altium-parser.test.mjs`: native sample parsing for schematic and PCB documents
...
- When changing sample-file expectations, verify them against the real corpus instead of weakening tests blindly
