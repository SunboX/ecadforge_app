...
7. WebMCP tools must not upload files, scan local paths, fetch remote resources, mutate app state, or expose raw file contents. Registered descriptors must mark the tools as read-only and untrusted-content capable, and runtime transport configuration must avoid wildcard origin allowlists.
